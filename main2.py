...
def lcd_init():
    ...
    try:
        ...
        print(f"[LCD] Failed to initialize LCD: {e}")
        LCD_ENABLED = False

def lcd_show(line1, line2=""):
    """Display a message on the LCD (max 16 chars per line)."""
...
def lcd_clear_after(seconds=3):
    """Clear the LCD after a delay (runs in background thread)."""
    def _clear():
        time.sleep(seconds)
...
def handle_access(name):
    ...
    if student is None:
        # ❌ Not registered
        lcd_show("NOT REGISTERED", name[:16])
        lcd_clear_after(3)
    ...
    if name in _last_action:
        ...
        if elapsed < ENTRY_EXIT_COOLDOWN:
            ...
            # ⏳ Cooldown active
            lcd_show(f"Wait {remaining}s", name[:16])
            lcd_clear_after(2)
    ...
    # ✅ Show access granted on LCD
    time_str = datetime.now().strftime('%H:%M:%S')
    if action == "ENTRY":
        lcd_show(f"Welcome!", name[:16])
    else:
        lcd_show(f"Goodbye!", name[:16])
    lcd_clear_after(4)
...
def recognize_worker(frame, bbox, face_key):
    try:
        ...
        if name != "Unknown":
            ...
            color = (0, 0, 255)
            # ❌ Unknown person on LCD
            lcd_show("ACCESS DENIED", "Unknown Person")
            lcd_clear_after(3)
            threading.Thread(target=red_alert, daemon=True).start()
